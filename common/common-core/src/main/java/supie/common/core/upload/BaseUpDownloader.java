...
public abstract class BaseUpDownloader {
    ...
    public static boolean containFile(String fileInfoJson, String filename) {
        ...
        JSONObject jsonArray = JSON.parseObject(fileInfoJson);
        String fileNameDb = jsonArray.getString("filename");
        return StringUtils.equals(filename, fileNameDb);
//        if (StringUtils.isAnyBlank(fileInfoJson, filename)) {
//            return false;
//        }
...
//        return false;
    ...
    protected UploadResponseInfo verifyUploadArgument(
            Boolean asImage, InputStream uploadFileInputStream) throws IOException {
        UploadResponseInfo responseInfo = new UploadResponseInfo();
        if (Objects.isNull(uploadFileInputStream)) {
            responseInfo.setUploadFailed(true);
            responseInfo.setErrorMessage(ErrorCodeEnum.INVALID_UPLOAD_FILE_ARGUMENT.getErrorMessage());
            return responseInfo;
        }
        if (BooleanUtil.isTrue(asImage) && ImageIO.read(uploadFileInputStream) == null) {
            responseInfo.setUploadFailed(true);
            responseInfo.setErrorMessage(ErrorCodeEnum.INVALID_UPLOAD_FILE_FORMAT.getErrorMessage());
            return responseInfo;
        }
     ...
     * @param response    Http 应答对象。
     */
    public abstract String obtainTheConfigurationFileContent(
            String rootBaseDir,
            String modelName,
            String fieldName,
            String fileName,
            Boolean asImage) throws IOException;
}
