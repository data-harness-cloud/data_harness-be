package supie.webadmin.app.dto;

import supie.common.core.validator.UpdateGroup;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;

import javax.validation.constraints.*;

/**
 * DefinitionDimensionPropertyColumnDto对象。
 *
 * @author rm -rf .bug
 * @date 2020-11-12
 */
@ApiModel("DefinitionDimensionPropertyColumnDto对象")
@Data
public class DefinitionDimensionPropertyColumnDto {

    /**
     * 租户号。
     */
    @ApiModelProperty(value = "租户号", required = true)
    @NotNull(message = "数据验证失败，租户号不能为空！", groups = {UpdateGroup.class})
    private Long id;

    /**
     * 数据所属人。
     */
    @ApiModelProperty(value = "数据所属人")
    private Long dataUserId;

    /**
     * 数据所属部门。
     */
    @ApiModelProperty(value = "数据所属部门")
    private Long dataDeptId;

    /**
     * 维度表id。
     */
    @ApiModelProperty(value = "维度表id")
    private Long dimensionId;

    /**
     * 属性名称。
     */
    @ApiModelProperty(value = "属性名称")
    private String propertyName;

    /**
     * 属性英文名称。
     */
    @ApiModelProperty(value = "属性英文名称")
    private String propertyEnName;

    /**
     * 数据类型。
     */
    @ApiModelProperty(value = "数据类型")
    private String propertyDataType;

    /**
     * 业务描述。
     */
    @ApiModelProperty(value = "业务描述")
    private String propertyDescription;

    /**
     * 小数点。
     */
    @ApiModelProperty(value = "小数点")
    private Integer propertyDecimalLength;

    /**
     * 是否主键。
     */
    @ApiModelProperty(value = "是否主键")
    private String isPrimary;

    /**
     * updateTime 范围过滤起始值(>=)。
     */
    @ApiModelProperty(value = "updateTime 范围过滤起始值(>=)")
    private String updateTimeStart;

    /**
     * updateTime 范围过滤结束值(<=)。
     */
    @ApiModelProperty(value = "updateTime 范围过滤结束值(<=)")
    private String updateTimeEnd;

    /**
     * createTime 范围过滤起始值(>=)。
     */
    @ApiModelProperty(value = "createTime 范围过滤起始值(>=)")
    private String createTimeStart;

    /**
     * createTime 范围过滤结束值(<=)。
     */
    @ApiModelProperty(value = "createTime 范围过滤结束值(<=)")
    private String createTimeEnd;

    /**
     * propertyDecimalLength 范围过滤起始值(>=)。
     */
    @ApiModelProperty(value = "propertyDecimalLength 范围过滤起始值(>=)")
    private Integer propertyDecimalLengthStart;

    /**
     * propertyDecimalLength 范围过滤结束值(<=)。
     */
    @ApiModelProperty(value = "propertyDecimalLength 范围过滤结束值(<=)")
    private Integer propertyDecimalLengthEnd;

    /**
     * property_name / property_en_name / property_data_type / property_description LIKE搜索字符串。
     */
    @ApiModelProperty(value = "LIKE模糊搜索字符串")
    private String searchString;
}
